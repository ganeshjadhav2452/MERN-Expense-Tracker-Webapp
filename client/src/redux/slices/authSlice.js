...
export const signInUser = (userDetails) => {
    ...
    return async (dispatch, getstate) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/user/signin', { ...userDetails })
...
export const getInitialUserDetails = () => {
    return async (dispatch, getState) => {
        try {
            const token = localStorage.getItem('token');
            const userDetails = await axios.get('http://localhost:5000/getinitialuserdetails', {
                headers: {
                    "Authorization": token,
...
export const sendEmailForResetPassword = (email) => {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/password/sendemailforresetpassword', { email: email }, {
...
export const resetpasswordApicall = (email, password, uuid) => {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            const response = await axios.post(`http://localhost:5000/resetpassword/${uuid}`, { password: password, email: email }, {
                headers: {
                    'Authorization': token,
                    'Content-Type': 'application/json',
                }
